    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(accessToken == null && refreshToken == null){
            ...
            if(StringUtils.hasText(refreshToken) && jwtUtils.validateToken(refreshToken)) {
                System.out.println("유효한 refresh 토큰입니다. URI: {}");
                //logger.debug("유효한 refresh 토큰입니다. URI: {}", requestURI);

                int roles = Integer.parseInt(jwtUtils.getRole(refreshToken));
                String userid = jwtUtils.getId(refreshToken);
                String name = jwtUtils.getName(refreshToken);

                String newAccessToken = jwtUtils.createAccessToken(roles, userid, name);

                //Header에 access token 정보 담아서 응답
                response.setHeader("Authorization", "Bearer " + newAccessToken);
            } else {
                // 리프래쉬 토큰 유효하지 않을 때
                System.out.println("유효하지 않은 refresh 토큰입니다. uri: {}");
                //logger.debug("유효하지 않은 refresh 토큰입니다. uri: {}", requestURI);
                return false;
            }
        }else{ // 액세스, 리프래쉬 토큰 둘 다 있을 때
            System.out.println("access, refresh 토큰 둘 다 존재합니다.");
            //logger.debug("access, refresh 토큰 둘 다 존재합니다.");
            if(jwtUtils.validateToken(accessToken)) {
                //accesstoke이 유효할 때
                System.out.println("유효한 access 토큰 정보입니다. URI: {}");
                //logger.debug("유효한 access 토큰 정보입니다. URI: {}", requestURI);
            } else {
                //refreshtoken 검증 수행
                System.out.println("유효하지 않은 access 토큰 정보입니다. refresh 토큰 검증이 필요합니다. URI: {}");
                ...
                if(StringUtils.hasText(refreshToken) && jwtUtils.validateToken(refreshToken)) {
                    System.out.println("유효한 refresh 토큰 정보입니다. URI: {}");
                    ...
                    //둘 다 유효하지 않을 때
                    System.out.println("유효하지 않은 JWT 토큰입니다. uri: {}");
